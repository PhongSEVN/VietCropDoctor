...
from dataset import TransformerImageDataset
...
def main():
    ...
    results_dir = _HERE.parent / "results" / "transformer"
    models_dir = results_dir / "models"
    results_dir.mkdir(parents=True, exist_ok=True)
    models_dir.mkdir(parents=True, exist_ok=True)

    train_ds = TransformerImageDataset(dataset_root / "train", cfg, image_size=cfg["dataset"]["image_size"], train=True)
    val_ds = TransformerImageDataset(dataset_root / "val", cfg, image_size=cfg["dataset"]["image_size"], train=False)
    test_ds = TransformerImageDataset(dataset_root / "test", cfg, image_size=cfg["dataset"]["image_size"], train=False)
    ...
    run_name = f"transformer-{model_name.split('/')[-1]}"
    ...
    try:
        ...
        mlflow.set_tag(
            "mlflow.note.content",
            f"{run_name} | imbalance={imb_strategy} | "
        ...
        try:
            ...
            mlflow.register_model(f"runs:/{run_id}/model_pytorch", "VietCropDoctor-vit")
        except Exception as exc:
