...
from dataset import MobileNetV3ImageDataset
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "mobilenetv3_training_config.yaml"
...
def main():
    ...
    results_dir = _HERE.parent / "results" / "mobilenetv3"
    models_dir = results_dir / "models"
    results_dir.mkdir(parents=True, exist_ok=True)
    models_dir.mkdir(parents=True, exist_ok=True)

    train_ds = MobileNetV3ImageDataset(dataset_root / "train", cfg, image_size=cfg["dataset"]["image_size"], train=True)
    val_ds = MobileNetV3ImageDataset(dataset_root / "val", cfg, image_size=cfg["dataset"]["image_size"], train=False)
    test_ds = MobileNetV3ImageDataset(dataset_root / "test", cfg, image_size=cfg["dataset"]["image_size"], train=False)
    ...
    mlflow.start_run(run_name="mobilenetv3")
    mlflow.set_tag("model_name", "mobilenetv3")
    mlflow.set_tags({
    ...
    try:
        ...
        plt.suptitle("MobileNetV3 — training curves")
        plt.tight_layout()
        ...
        mlflow.set_tag(
            "mlflow.note.content",
            f"mobilenetv3 | imbalance={imb_strategy} | "
        ...
        try:
            ...
            mlflow.register_model(f"runs:/{run_id}/model_pytorch", "VietCropDoctor-mobilenetv3")
        except Exception as exc:
