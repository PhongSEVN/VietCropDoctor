...
Usage (config defaults to ml/data-pipeline/spark/spark_config.yaml):
    python ml/preprocessing/spark_pipeline.py --split train
    spark-submit ml/preprocessing/spark_pipeline.py \
        --config ml/data-pipeline/spark/spark_config.yaml \
        --split train

Requires: pip install pyspark Pillow pyyaml
...
if __name__ == "__main__":
    # Default config lives at repo's ml/data-pipeline/spark/spark_config.yaml,
    # resolved relative to this file so the command works from any CWD.
    _DEFAULT_CONFIG = Path(__file__).resolve().parent.parent / "data-pipeline" / "spark" / "spark_config.yaml"
